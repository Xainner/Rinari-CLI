...
def test_local_image_turn_persists_pixels_and_large_preview(
    ...
    path = tmp_path / "imagen con espacios.png"
    Image.new("RGB", (1800, 1000), "purple").save(path)
    session_id = _create_chat(server, tmp_path, "visual")
    services.sessions.set_permission(session_id, "full-access")
    ...
    restored = agent_runtime._restore_history(services, services.sessions.show(session_id))
    image_message = next(m for m in restored if m.tool_call_id == "view")
