...
def test_reported_visual_pressure_can_compact_media_without_dropping_text(app_ctx):
    ...
    history = [
        ChatMessage(
            role="user",
            content=str(n),
            images=(SimpleNamespace(uri=f"artifact://s/media/{n}.png", sha256=str(n)),),
        )
        for n in range(3)
    ]
    context = AgentContext(
        session_id="visual-pressure",
        model_ref="m",
        tool_ctx=None,
        assembler_base=AssemblerContext(),
    ...
    service = ContextService(app_ctx, ArtifactStore(app_ctx))
    assert service.maybe_compact(
        context, session_id="visual-pressure", window_tokens=100_000, used_input_tokens=90_000
    )
