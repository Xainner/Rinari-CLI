...
def test_complete_subagent_report_reaches_coordinator(monkeypatch, git_repo):
    import json

    from rinari.agents.orchestrator import AgentOrchestrator
    from rinari.agents.tools import _result_dict
    ...
    assert _result_dict(result)["summary"] == report

    class CompletedRunner:
        def run(self, spec):
            return result

    orchestrator = AgentOrchestrator(CompletedRunner())
    orchestrator.spawn("explore", "Map the repository")
    delivered = orchestrator.collect_for_final(CancellationToken())
    assert json.loads(delivered)[0]["summary"] == report
