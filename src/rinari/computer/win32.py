...
def descendant_texts(hwnd: int) -> list[tuple[str, str]]:
    """(class, text) of edit-like controls at any depth. Lab verification helper."""
    ...
    def callback(child, _param):
        ...
        if cls.value in ("Edit", "RichEditD2DPT", "RICHEDIT50W", "RichEdit20W"):
            user32.GetWindowTextW(child, buf, buf_len)
            if buf.value:
