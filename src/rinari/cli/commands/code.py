"""CLI → Code handoff (Phase 11): ``rinari code [path]``.

Opens the desktop client on the same project/session. The desktop binary
is resolved via RINARI_CODE_BIN or PATH (`rinari-code`); the engine never
...
def _resolve_binary(explicit: str | None) -> str:
    if explicit:
        ...
        raise InvalidUsageError(
            ...
            hint="Set RINARI_CODE_BIN to the rinari-code executable.",
        )
    from_env = os.environ.get("RINARI_CODE_BIN")
    if from_env:
        return _resolve_binary(from_env)
    found = shutil.which("rinari-code")
    if found:
        return found
    raise InvalidUsageError(
        "Desktop client not found.",
        hint="Install Rinari Code or set RINARI_CODE_BIN.",
    )


@with_error_handling("code")
def code(
    ctx: typer.Context,
    ...
    binary: str | None = typer.Option(
        None, "--binary", help="Override the rinari-code executable."
    ),
) -> None:
    ...
    try:
        ...
        raise InvalidUsageError(f"Could not launch Rinari Code: {exc}") from exc
    typer.echo(f"Opening Rinari Code: {target}")
